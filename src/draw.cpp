...
void drawTriangle(Vertex v0, Vertex v1, Vertex v2, glm::vec3 color) {
    float eps = 0.0001;
    v0.position += eps * v0.normal;
    v1.position += eps * v1.normal;
    v2.position += eps * v2.normal;
...
void drawVector(const glm::vec3& origin, const glm::vec3& vector, const glm::vec3& color) {
    Ray ray;
    ray.direction = vector;
    ray.origin = origin;
    ray.t = glm::length(vector);

    // draw vector with green color
    drawRay(ray, color);
}